<div align="center">

![header](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,2,2,5,30&height=300&section=header&text=Namgung%20Geonwoo&fontSize=70&fontColor=fff&animation=twinkling&fontAlignY=40&desc=AI%20Developer%20and%20Backend%20Engineer&descAlignY=65&descAlign=50)

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&width=600&lines=🏆+Award-winning+AI+Developer;🤖+RAG+%26+LLM+Specialist;🚀+Backend+Engineer;💡+Problem+Solver+%26+Innovator;🎯+Building+AI+for+Social+Impact)](https://git.io/typing-svg)

</div>

```javascript
const NamgungGeonwoo = {
    location: "Seoul, Korea 🇰🇷",
    currentFocus: ["RAG Systems", "Multi-Agent AI", "Backend Development"],
    achievements: ["🥇 Ministry Award Winner", "🤖 AI Specialist", "👨‍💻 Team Leader"],
    philosophy: "Building the future with AI • Solving real-world problems • Always learning"
};
```

## 🏆 주요 성과

<table>
<tr>
<td width="50%">

### 🥇 Award Winner
**제7회 k-DigitalTraining Hackathon**
- **🏆 고용노동부장관상 수상**
- **💡 프로젝트**: "고향으로 ON"
- **🎯 임팩트**: 중장년층 고향 정착 지원
- **🛠️ 기술**: RAG, React Native, FastAPI

</td>
<td width="50%">

### 🚀 Tech Achievements
- **🤖 6개 AI/ML 프로젝트** 성공적 완료
- **👥 팀 리더십** 경험 다수
- **📊 성능 향상** F1 Score 11.4% 개선
- **⚡ 빠른 개발** 2일만에 MVP 구현

</td>
</tr>
</table>

## 🚀 주요 프로젝트

### 🤖 AI & ML 프로젝트

#### 1. 🏠 [고향으로 ON](https://github.com/NGGW519/7th-kDT-HACKATHON)
*🥇 제7회 k-DigitalTraining Hackathon 고용노동부장관상 수상작*
- **기술**: React Native, TypeScript, FastAPI, OpenAI GPT, RAG, ChromaDB, MySQL
- **핵심 성과**:
  - 전국 해커톤 대회에서 **고용노동부장관상** 수상
  - 중장년층 디지털 접근성을 고려한 **음성 기반 UI/UX** 구현
  - **RAG 기술**로 지역별 맞춤형 정보 제공 시스템 구축
  - **게이미피케이션** 요소로 지역사회 적응 동기 부여
- **사회적 임팩트**: 중장년층 고향 정착 지원을 통한 지방 소멸 문제 해결 기여
- **역할**: 팀장, AI 시스템 설계 및 구현

#### 2. 💼 [CLIKCA - AI 업무 파트너](https://github.com/SKNETWORKS-FAMILY-AICAMP/SKN13-FINAL-1TEAM)
*SK Networks Family AI Camp 13기 최종 프로젝트*
- **기술**: Electron, React, FastAPI, LangGraph, OpenAI text-embedding-3-large, ChromaDB, AWS S3
- **핵심 성과**:
  - **멀티 에이전트 시스템** 구축으로 복잡한 업무 자동화 구현
  - **하이브리드 검색** (키워드 + 벡터) 기술로 정확한 문서 검색
  - **LangGraph** 활용한 지능형 워크플로우 라우팅
  - 신입사원 업무 적응 시간 단축을 위한 **자연어 업무 처리**
- **기술적 혁신**: Agent별 전문화된 역할 분담 (검색, 편집, 작성)
- **역할**: AI 워크플로우 아키텍처 설계 및 LangGraph 구현

#### 3. 🏥 [의학 논문 기반 팩트체킹 챗봇](https://github.com/SKNETWORKS-FAMILY-AICAMP/SKN13-3rd-1TEAM)
*의학 정보 신뢰성 검증 AI 시스템*
- **기술**: GPT-4.1, ChromaDB, RAG, RAGAS 평가 프레임워크, Streamlit
- **핵심 성과**:
  - **3개 주요 의학 데이터베이스** 통합 (Europe PMC, PubMed, MedRxiv)
  - **RAGAS 자동 평가**로 RAG 성능 객관적 측정 시스템 구축
  - 일반 상식과 **최신 의학 연구** 간 차이점 자동 분석
  - **실시간 의학 논문** 검색 및 팩트체킹 기능
- **사회적 가치**: 의료 미신 및 잘못된 건강 정보 확산 방지
- **역할**: RAG 파이프라인 설계 및 RAGAS 평가 시스템 구현

### 🌐 웹 서비스 프로젝트

#### 4. 🏙️ [지금, 서울 - AI 시정 Q&A](https://github.com/SKNETWORKS-FAMILY-AICAMP/SKN13-4th-1TEAM)
*서울시 공공데이터 기반 AI 시민 서비스*
- **기술**: Django, LangChain, LangGraph, ChromaDB, MySQL, Seoul Open Data API
- **핵심 성과**:
  - **서울 열린데이터 광장** API 다중 통합으로 실시간 도시 정보 제공
  - **지능형 질문 분류** 시스템으로 적절한 정보원 자동 라우팅
  - **FAQ 자동 응답** 및 **관리자 대시보드** 구현
  - **반응형 웹 디자인**으로 모바일/데스크톱 최적화
- **시민 편의성**: 복잡한 행정 정보를 자연어로 쉽게 접근
- **개발 기간**: 2일 (2025.07.21-22) 단기 집중 개발

#### 5. 🚗 [DRMC - 자동차 리콜 정보 서비스](https://github.com/SKNETWORKS-FAMILY-AICAMP/SKN13-1st-2Team)
*"결함도 정보다" - 투명한 자동차 구매 의사결정 플랫폼*
- **기술**: Streamlit, MySQL, Selenium, Python, Pandas
- **핵심 성과**:
  - **자동차 리콜 정보 시각화**로 숨겨진 결함 정보 투명 공개
  - **사용자 맞춤 추천 시스템**으로 구매 기준별 차량 매칭
  - **웹 크롤링 자동화**로 실시간 자동차 정보 수집
  - **브랜드별 카테고리** 체계적 분류 및 상세 스펙 제공
- **소비자 가치**: 정보 비대칭성 해소로 현명한 자동차 구매 지원
- **개발 기간**: 2일 (2025.04.09-10) 초단기 MVP 개발
- **팀 구성**: 5인 협업 프로젝트

### 📊 데이터 사이언스 프로젝트

#### 6. 🏥 [환자 이탈 예측 모델](https://github.com/SKNETWORKS-FAMILY-AICAMP/SKN13-2nd-7Team)
*"도망가지마!!!!" - LAMA 환자 예측 ML 모델*
- **기술**: Python, XGBoost, SMOTE, Scikit-learn, Pandas, NumPy
- **핵심 성과**:
  - **F1 Score 11.4% 향상** (0.4624 → 0.5153) 달성
  - **클래스 불균형 해결**: SMOTE 기법으로 소수 클래스 성능 개선
  - **임계값 최적화**: Precision-Recall 트레이드오프 분석
  - **핵심 특성 발견**: 성별, 연령, 정신건강, 약물남용 등 주요 인자 식별
- **의료진 가치**: 환자 이탈 위험도 사전 예측으로 적극적 개입 가능
- **기술적 도전**: 샘플 크기 최적화와 임계값 튜닝으로 성능 극대화
- **팀명**: "도망가지마!!!!" (Don't Run Away!!!!) 🚑

<div align="center">

## 🛠️ Tech Arsenal

*"Building the future, one line of code at a time"*

</div>

<table>
<tr>
<td align="center" width="25%">

### 🤖 AI & ML
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-121212?style=for-the-badge&logo=chainlink&logoColor=white)
![ChromaDB](https://img.shields.io/badge/ChromaDB-FF6B6B?style=for-the-badge&logo=database&logoColor=white)

</td>
<td align="center" width="25%">

### 💻 Backend
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white)

</td>
<td align="center" width="25%">

### 🎨 Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![Electron](https://img.shields.io/badge/Electron-2B2E3A?style=for-the-badge&logo=electron&logoColor=9FEAF9)

</td>
<td align="center" width="25%">

### 🗄️ Database & Cloud
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)

</td>
</tr>
</table>

### 🎯 Specialized Skills

<table>
<tr>
<td width="50%">

**🤖 AI Development**
- RAG Systems
- Multi-Agent AI
- LLM Integration

**💻 Backend Development**
- FastAPI/Django
- Database Design
- API Architecture

</td>
<td width="50%">

**🔬 Data Science**
- ML Modeling
- Performance Optimization
- Feature Engineering

**👥 Leadership**
- Team Management
- Project Planning
- Technical Mentoring

</td>
</tr>
</table>

<div align="center">

## 📊 GitHub Analytics

<table>
<tr>
<td width="50%">

![NGGW519's GitHub stats](https://github-readme-stats.vercel.app/api?username=NGGW519&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117)

</td>
<td width="50%">

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=NGGW519&layout=compact&theme=radical&hide_border=true&bg_color=0D1117)

</td>
</tr>
</table>

<table>
<tr>
<td width="50%">

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=NGGW519&theme=radical&hide_border=true&background=0D1117)

</td>
<td width="50%">

![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=NGGW519&theme=react-dark&hide_border=true&bg_color=0D1117&color=79fe96&line=00d4aa&point=ffffff)

</td>
</tr>
</table>

### 📈 Contribution Activity
![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=NGGW519&theme=github-compact&hide_border=true&bg_color=0D1117&color=79fe96&line=00d4aa&point=ffffff)

</div>

## 🎯 전문 분야

- 🤖 **RAG & LLM**: 검색 증강 생성 및 대화형 AI 시스템 구축
- 🏗️ **AI 워크플로우**: LangGraph를 활용한 멀티 에이전트 시스템
- 💻 **백엔드 개발**: FastAPI, Django 기반 API 설계 및 구현
- 📊 **데이터 사이언스**: 머신러닝 모델링 및 성능 최적화

## 🔭 현재 관심사

- 🧠 대화형 AI와 RAG 시스템 고도화
- 🎨 멀티모달 AI (텍스트, 이미지, 음성 통합)
- 🎮 게이미피케이션 기반 사용자 경험 설계
- 🗣️ 음성 인터페이스 및 접근성 향상
- 🌐 실시간 데이터 처리 및 API 통합

<div align="center">

## 📫 Let's Connect!

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/NGGW519)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:srrd1357@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/yourprofile)


### 🎮 When I'm Not Coding

<table>
<tr>
<td align="center" width="33%">

🤖 **Building AI**
*Experimenting with new RAG techniques*

</td>
<td align="center" width="33%">

📚 **Learning**
*Always exploring cutting-edge tech*

</td>
<td align="center" width="33%">

🌱 **Growing**
*Mentoring junior developers*

</td>
</tr>
</table>

---

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,2,2,5,30&height=100&section=footer"/>

**⭐️ "Building tomorrow's solutions, today!"**

*Thanks for visiting! Don't forget to ⭐ my repositories if you find them interesting!*

</div>
